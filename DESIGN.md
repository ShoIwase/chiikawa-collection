...
alias "live" ← バージョン管理で SnapStart が有効になる
    ...
    └── PUT  /items/{name}/tags   → DynamoDB Update (タグ編集)
...
| `AreaType` | String | `都道府県` / `市区町村`（箱根等の温泉地含む） / `その他`（海外・広域・不明） |
...
フロントエンドから直接 SRP 認証 (共有パスワードのみ入力。ユーザー名は `family` 固定で
家族全員が同じアカウントを使う)。
...
| 画面 | URL | 説明 |
|---|---|---|
| ログイン | `/login/` | 共有パスワードのみ入力（ユーザー名は `family` 固定） |
| コレクション一覧 | `/collection/` | メイン画面。アイテムをタップして所持切替 |
| 未確認レビュー | `/verify/` | スクレイパーが追加した新着の確認・修正 |
| 集計 | `/stats/` | キャラ別・都道府県別の所持率などの集計表示 |

### コレクション一覧の機能
- **アラート**: 未確認アイテムが N 件ある場合に通知バナーを表示
- **フィルター**: 「絞り込み」ボタンで開閉する折りたたみパネル。都道府県 → 市区町村の2段選択、
  キャラクター（複数選択）、タグ、❤️欲しい／⭐お気に入り／未所持のみのトグルで絞り込める
- **並べ替え**: 名前順・地域順・キャラ順・所持済み優先・未所持優先
- **検索**: アイテム名・エリアのテキスト検索
- **切替**: タップで所持/未所持をローカルに切り替え（未保存は琥珀色リングで表示）。
  画面下部の「保存」ボタンでまとめて `PUT /items/{name}/status` を確定する
  （誤タップ防止のため、タップだけではサーバーに反映されない）
- **タグ編集・お気に入り**: 自由タグの追加編集、❤️欲しい／⭐お気に入りは保存ボタンを介さず即時反映
...
画像から地域が取れた場合は `area_mapping.classify_area()`、取れない場合は商品名 (`ItemName`)
から `predict_area()` でエリア種別（`AreaType`）とエリア名（`AreaName`）を推測する。
`AreaType` は常に `都道府県` / `市区町村` / `その他` の3分類のいずれかを返す。

| 優先順位 | 判定方法 | 例 |
|---|---|---|
| 1 | 都道府県名が含まれる | `"北海道 ダイカットキーホルダー"` → `都道府県 / 北海道` |
| 2 | 海外地名が含まれる | `"香港 ダイカットキーホルダー"` → `その他 / 香港` |
| 3 | 温泉地・観光地名が含まれる | `"箱根 ダイカットキーホルダー"` → `市区町村 / 箱根` |
| 4 | フォールバック | 最初のトークンから地名を推測 → `市区町村 / xxx` |
